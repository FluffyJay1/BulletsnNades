...
namespace BulletsnNades
{
    class bullet4 : GameObject
    {
        ...
        void Explode()
        {
            Position.X = prevPosX;
            Position.Y = prevPosY;
            for(float i = 0; i <= 360; i += 360 / PersistentData.fragmentCount)
            {
                float tempVX = (float)Math.Cos(i);
                float tempVY = (float)Math.Sin(i);
                tempVX *= 6; //originally *6
                tempVY *= 6;
                GameObject b = new fragment(tempVX,tempVY);
                b.Position = Position;
            }
            this.IsDead = true;
        }
